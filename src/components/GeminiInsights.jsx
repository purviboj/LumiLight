...
export default function GeminiInsights({
  ...
  const nearestResult = useMemo(() => {
    ...
    const activeLocation = selectedLocationOverride || userPosition;
    return findNearestLightPollutionPoint(points, activeLocation, selectedYear);
  }, [userPosition, selectedYear, selectedLocationOverride]);

  const context = useMemo(() => {
    if (!nearestResult) return null;

    const activeLocation = selectedLocationOverride || userPosition;
    const localSnapshot = activeLocation
      ? getLocalSkySnapshot({ location: activeLocation, selectedYear, radiusKm: 250 })
      : null;
    ...
    const nearbyLowerPollution = (localSnapshot?.stargazing?.rankedNearbyLocations || [])
      .filter((place) => place.intensity < rawIntensity)
      .slice(0, 5)
      .map((place) => ({
        name: place.name,
  ...
  }, [nearestResult, restoreNightMode, selectedLocationOverride, selectedYear, userPosition]);
  ...
  async function handleGenerate() {
    ...
    try {
      const result = await generateGeminiInsights({
        ...
        nearbyLowerPollution: context.nearbyLowerPollution,
          ...
          <div className="space-y-3">
            ...
            {insights && !loading && (
              <div className="insight-fade-in space-y-2 rounded-lg border border-white/10 bg-black/20 p-3 text-sm">
                ...
                <div>
                  <h3 className="font-semibold text-slate-100">Why light pollution is high here</h3>
                  <p className="text-slate-300">{insights.pollutionCauseSummary}</p>
                </div>
                <div>
                ...
                <div>
                  <h3 className="font-semibold text-slate-100">Nearby places with less light pollution</h3>
                  {insights.nearbyDarkerPlaces.length > 0 ? (
                    <ul className="list-disc space-y-1 pl-5 text-slate-300">
                      {insights.nearbyDarkerPlaces.map((place, index) => (
                        <li key={`${place}-${index}`}>{place}</li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-slate-300">No better nearby spots were identified in the current radius.</p>
                  )}
                </div>
                <div>
